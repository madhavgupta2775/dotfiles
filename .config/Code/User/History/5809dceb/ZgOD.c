...
{
    for (thread_count = 1; thread_count <= 1; thread_count <<= 1) {
        pthread_barrier_init(&bar, NULL, thread_count);
        m = 8000000, n = 8; printf("m = %d, n = %d\n", m, n); driver(); printf("\n");
        m = 8000, n = 8000; printf("m = %d, n = %d\n", m, n); driver(); printf("\n");
        m = 8, n = 8000000; printf("m = %d, n = %d\n", m, n); driver(); printf("\n");
        printf("==================================================================\n\n");
...
void *Pth_mat_vect(void *rank)
{
    ...
    int *cum = (double *)malloc(local_m * sizeof(int));
    if (cum == NULL) {
    ...
    for (i = my_first_row; i < my_last_row; i++)
    {
        // y[i] = 0.0;
        printf("%d \n", i - my_first_row);
        cum[i - my_first_row] = 0;
        for (j = 0; j < n; j++)
        {
            temp = A[sub++];
            temp *= x[j];
            cum[i - my_first_row] += temp;
        }
        
        pthread_barrier_wait(&bar);
        y[i] = cum[i - my_first_row];
    }
    GET_TIME(finish);

    double gflops = (2.0 * m * n) / ((finish - start) * 1e9);
    printf("Thread %ld > Elapsed time = %e seconds, GFlops/sec = %lf\n",
           my_rank, finish - start, gflops);
